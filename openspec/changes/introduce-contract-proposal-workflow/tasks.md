...
- [ ] 1.1 Add minimal `contract.contract` and `contract.proposal` models, native
  views/actions, amount/currency fields, state fields, mail-thread integration as
  needed, and declared upstream dependencies.
- [ ] 1.2 Add native groups, ACLs, record rules, and narrow model transitions so
...
- [ ] 1.3 Implement Proposal draft, submission, withdrawal, rejection, and
...
- [ ] 2.1 Implement authorized, transactional, idempotent Proposal acceptance;
...
- [ ] 2.2 Link Project, Contract, and accepted Proposal; keep Proposal amount and
...
- [ ] 2.3 Remove or simplify Project recruitment fields, candidate authority,
  unassigned Contractor discovery, and duplicate compensation behavior so
  Contractors access Projects only after award.
- [ ] 2.4 Add an idempotent upgrade migration: convert compatible legacy assigned
...
- [ ] 3.1 Add focused Odoo tests for Contract/Proposal state, commercial-entity
  customer ownership, Contractor and competitor isolation, invitation, and
  Proposal terms authorization.
- [ ] 3.2 Add focused Odoo tests for acceptance atomicity/idempotence, competing
  ...
  regression coverage.
- [ ] 3.3 Run clean install and upgrade verification on disposable databases,
  then run the focused test suite without browser automation.
- [ ] 3.4 Update OpenSpec tasks only after evidence, sync final requirements,
